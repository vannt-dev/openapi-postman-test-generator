...
const boundary = requests.find(request => request.name.includes('[Negative] createProfile - out-of-range age'));
assert.ok(boundary);
assert.equal(JSON.parse(boundary.request.body.raw).age, 17);
assert.ok(boundary.event[0].script.exec.some(line => line.includes('Out-of-range value')));
